	...
	**/
	this.number_format = function(number,decimals,dec_point,thousands_sep) {
		...
		*/
		...
		//Obtengo decimales
		if(number % 1 !=0){//La cantidad tiene decimales
			var str = number.toString().split('.');
			var cantidad=str[0];
			output = cantidad.toString().match(/(\d+?)(?=(\d{3})+(?!\d)|$)/g); //Se divide la cantidad cada 3 digitos
			
			if(cantidad>=100000){//Cuando es más de un millón se omiten las unidades
				//Obtener la posición de las unidades
			...
			output = number.toString().match(/(\d+?)(?=(\d{3})+(?!\d)|$)/g); //Se divide la cantidad cada 3 digitos
    		
    		if(number>=1000000){//Cuando es más de un millón se omiten las unidades
    			//Obtener la posición de las unidades
    			var pos=output.length-1;
    			output[pos]='000';
    			number=output.join();
